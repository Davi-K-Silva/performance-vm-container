...
#
# Run every benchmarks/*/install.sh that exists.
...
DRY_RUN=0         # set to 1 to show what would run

print_help() {
  cat <<'EOF'
Usage: scripts/install_all.sh [--stop-on-error] [--dry-run] [--help]

  --stop-on-error   Stop at the first failing install.sh
...
while IFS= read -r -d '' BENCH_PATH; do
  ...
  INSTALL="$BENCH_PATH/install.sh"

  if [[ -f "$INSTALL" ]]; then
    echo ""
    echo "--> [$BENCH_NAME] Found install.sh"

    if (( DRY_RUN )); then
      echo "DRY-RUN: bash \"$INSTALL\""
      SKIPPED+=("$BENCH_NAME (dry-run)")
      continue
    fi

    # Run via bash to avoid relying on +x bit
...
printf "Skipped (no install.sh or dry-run): %d\n" "${#SKIPPED[@]}"
((${#SKIPPED[@]})) && printf '  - %s\n' "${SKIPPED[@]}"
